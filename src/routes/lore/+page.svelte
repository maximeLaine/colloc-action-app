												...
												{#each entry.attachments as att}
													{#if att.type === 'image'}
														<a href={att.url} target="_blank" class="att-img-link">
															<img src={att.url} alt={att.name} class="att-img" loading="lazy" decoding="async" />
...
<style>
	...
	.lore-section { margin-bottom: 3rem; }
	...
	.category-title {
		font-size: 0.85rem; letter-spacing: 0.1em; color: #C2374A;
		margin: 1.5rem 0 0.75rem; padding-left: 0.75rem;
		border-left: 3px solid #C2374A;
		font-family: 'Cinzel', serif; text-transform: uppercase; font-weight: 700;
	}

	.lore-grid { display: flex; flex-direction: column; gap: 0.5rem; margin-bottom: 0.5rem; }
	.lore-card { padding: 0; overflow: hidden; }
	.lore-header { width: 100%; background: none; border: none; color: inherit; display: flex; align-items: center; justify-content: space-between; padding: 0.85rem 1.25rem; cursor: pointer; text-align: left; }
	.lore-header:hover { background: rgba(255,255,255,0.02); }
	.entry-title { font-family: 'Cinzel', serif; font-size: 0.85rem; font-weight: 700; letter-spacing: 0.05em; text-transform: uppercase; }
