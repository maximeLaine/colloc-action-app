...
{#if showModal}
	...
	<div class="drawer">
		...
		<h2 class="modal-title">
			{editing ? `Session ${editing.number} — Modifier` : 'Nouvelle Session'}
		</h2>
			...
			<div class="form-grid">
				<div class="field">
					...
					<input
						...
						type="number"
						min="1"
						value={editing?.number ?? data.sessions.length + 1}
					/>
				</div>
				<div class="field">
					...
					<input
						...
						name="date_played"
						type="date"
						value={toInputDate(editing?.date_played ?? null)}
					/>
				</div>
				<div class="field full">
					...
					<input
						...
						name="campaign"
						type="text"
						value={(editing as any)?.campaign ?? 'Colloc-Action'}
				...
				<div class="field full">
					<div class="summary-field-header">
						...
						<button
							type="button"
							class="btn-ai-toggle"
							onclick={() => (showAiSection = !showAiSection)}
						>
						...
						<div class="ai-summary-block">
							...
							<div class="ai-sum-actions">
								<button
									...
									disabled={aiSummaryLoading || !aiRawNotes.trim()}
								>
							...
							{#if generatedSummary}
								<div class="ai-summary-preview">
									...
									<button type="button" class="btn-use-summary" onclick={useSummary}
					...
					<textarea
						...
						bind:this={summaryEl}>{editing?.summary ?? ''}</textarea
					>
				</div>
				<div class="field full">
					...
					<textarea
						...
						name="dm_notes"
						rows="4"
						placeholder="Coulisses, révélations à venir...">{editing?.dm_notes ?? ''}</textarea
					>
				</div>
				<div class="field">
					...
					<input
						...
						type="number"
						min="0"
						value={editing?.xp_awarded ?? 0}
					/>
				</div>
				<div class="field">
					<label for="m-vis">Visibilité</label>
					<select id="m-vis" name="visibility">
						<option value="dm_only" selected={editing?.visibility === 'dm_only'}
							>🔒 MJ uniquement</option
						>
						<option value="players" selected={!editing || editing.visibility === 'players'}
							>👥 Joueurs</option
						>
