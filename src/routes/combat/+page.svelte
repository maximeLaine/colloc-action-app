	...
	{#if sheetMonster}
		...
		<div class="monster-sheet">
			...
			{#if sheetMonster.image_url}
				<img
					src={sheetMonster.image_url}
					alt={sheetMonster.name}
					class="sheet-img"
					loading="lazy"
					decoding="async"
				/>
			...
			<div class="sheet-header">
				...
				{#if sheetMonster.size || sheetMonster.type || sheetMonster.alignment}
					<p class="sheet-type">
						{[sheetMonster.size, sheetMonster.type, sheetMonster.alignment]
							.filter(Boolean)
							.join(' · ')}
					</p>
				{/if}
			...
			<div class="sheet-stats">
				{#if sheetMonster.cr}<div class="sheet-stat">
						<span class="stat-label">FP</span><span class="stat-val">{sheetMonster.cr}</span>
					</div>{/if}
				{#if sheetMonster.hp}<div class="sheet-stat">
						<span class="stat-label">PV</span><span class="stat-val">{sheetMonster.hp}</span>
					</div>{/if}
				{#if sheetMonster.ac}<div class="sheet-stat">
						<span class="stat-label">CA</span><span class="stat-val">{sheetMonster.ac}</span>
					</div>{/if}
				{#if sheetMonster.speed}<div class="sheet-stat">
						<span class="stat-label">VIT</span><span class="stat-val" style="font-size:0.75rem"
					...
					{#each [['FOR', sheetMonster.str_score], ['DEX', sheetMonster.dex_score], ['CON', sheetMonster.con_score], ['INT', sheetMonster.int_score], ['SAG', sheetMonster.wis_score], ['CHA', sheetMonster.cha_score]] as [lbl, val]}
						<div class="ability-score">
							...
							{#if val != null}<span class="as-mod"
									>({Math.floor((+val - 10) / 2) >= 0 ? '+' : ''}{Math.floor(
										(+val - 10) / 2
									)})</span
								>{/if}
						</div>
			...
			{#if sheetMonster.saving_throws || sheetMonster.skills_text || sheetMonster.damage_resistances || sheetMonster.damage_immunities || sheetMonster.condition_immunities || sheetMonster.senses || sheetMonster.languages}
				<div class="sheet-props">
					...
					{#if sheetMonster.senses}<p>
							<span class="prop-lbl">Sens</span>
							{sheetMonster.senses}
						</p>{/if}
									...
									<td>
										<form method="POST" action="?/deleteKill" use:enhance>
											...
											<button
												...
												type="submit"
												onclick={(e) => {
													if (!confirm('Supprimer ce kill ?')) e.preventDefault();
												}}>✕</button
											>
										</form>
									</td>
...
<style>
	...
	.monster-sheet {
		...
		max-width: 90vw;
		background: #0c0c0c;
		border-left: 1px solid rgba(194, 55, 74, 0.3);
		z-index: 300;
		overflow-y: auto;
