...
export async function buildCampaignContext(supabase: SupabaseClient): Promise<string> {
	const [sessionsResult, npcsResult] = await Promise.all([
		supabase
			.from('sessions')
	...
	const sessionLines = sessions
		.map(
			(s) =>
				`- Session ${s.number}${s.title ? ` "${s.title}"` : ''}${s.date_played ? ` (${s.date_played})` : ''}: ${s.summary || 'Pas de résumé.'}`
		)
		.join('\n');
	...
	return `## Contexte de la campagne "La Kolok-Action"
